...
```{r data_summary_gg, fig.cap = violin_cap, fig.height=8, fig.width=6}
LongData <- 
    ...
    mutate(type = case_when(
                    ...
                    TRUE ~ 'Weather' ), 
           var = recode(var, 
                        AirTemp = 'Air~temperature~(degree*C)', 
                        RH = 'Relative~humidity~(\'%\')', 
                        dpC = 'Dew~point~(degree*C)', 
                        FuelMoisture = 'Total~fuel~moisture~(\'%\')', 
                        tHa = 'Fuel~load~(kg~m^-2)',
                        MaxC = 'Flame~temp~(degree*C)', 
                        WindSpeed = 'Wind~speed~(m~s^-1)', 
                        ros = 'Spread~rate~(m~min^-1)', 
                        SoilMaxC = 'Soil~surface~temp~(degree*C)'), 
