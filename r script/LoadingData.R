...
wd_fp = "C:/Users/Devan.McGranahan/GoogleDrive/FireScienceDIY/SpatialFireBehavior/data/"
...
soil <-  
  read_xlsx("./data/fromMZ/Research_Fire_Database.xlsx", "Soil_TC") %>%
    select(-AirTemp, -RH, -WindSpd, -MaxWindSpd, -WindDir) %>%
  ...
  select(FireCode, plot, array, SoilMaxC) %>%
    filter(SoilMaxC != is.na(.))

# save(soil, file="./r objects/soil.Rdata")
